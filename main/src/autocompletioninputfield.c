...
char* displayAutoCompletionInputField(
	const AutoCompletionInputField inputField, const Database* restrict const words
) {
	...
	while (run) {
		symbol = runCharOfAutoCompletionInputField(
			...
			buffer, words, &indexOnPressedEnter
		);
		...
		if (symbol == -1) {
			...
			buffer = databaseGetKeyByIndex(words, indexOnPressedEnter);
			run = false;
		}
...
static char runCharOfAutoCompletionInputField(
	...
	const Database* const restrict words, int* const restrict indexOnPressedEnter
) {
	...
	unsigned int currentLineLength = changeAutoCompletionWord(startInputX, startInputY, buffer, 0, words, indexes[0]);
	cursorMoveTo(startInputX + bufferLength, startInputY);
